...
etc/                              hand-edits under /etc: sddm autologin+theme, swappiness, paru,
                                  sudoers rule for usr/local/bin/dp-replug
usr/local/bin/                    root-owned scripts: dp-replug (fake a DP replug for the FO48U)
...
3. Apply configs and units: `./export_current`, then enable everything in
   ...
   sudo install -o root -g root -m 0755 usr/local/bin/dp-replug /usr/local/bin/
   sudo install -o root -g root -m 0440 etc/sudoers.d/dp-replug /etc/sudoers.d/
   ```
